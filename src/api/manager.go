...
func (manager *Manager) AddRecord(request AddRecordRequest) (*AddRecordResponse, error) {
	...
	return &AddRecordResponse{BlockId: len(manager.Nodes[0].Blocks), Index: 0}, nil
}
...
func (manager *Manager) GetBlock(blockId int) (*GetBlockResponse, error) {
	...
	return &GetBlockResponse{BlockData: manager.Nodes[0].Blocks[blockId]}, nil
}
...
func (manager *Manager) GetLocalBlockchain(index int) (*GetLocalBlockchainResponse, error) {
	...
	response := GetLocalBlockchainResponse{Blocks: manager.Nodes[index].Blocks}
...
func (manager *Manager) GetBlockCount() int {
	count := 0
	for i := range manager.Nodes[0].Blocks {
...
func isBlockConfirmed(manager *Manager, index int) bool {
	...
	if (len(manager.Nodes[0].Blocks) <= index) {
		return false
	}
	block := manager.Nodes[0].Blocks[index]

	for _, node := range manager.Nodes {
		if (len(node.Blocks) <= index) {
			return false
		}

		if !bytes.Equal(node.Blocks[index].PoW, block.PoW) {
			return false
		}
	}
